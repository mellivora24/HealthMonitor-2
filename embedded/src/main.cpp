...
#define FALL_ACCEL_THRESHOLD 2.5  // Ngưỡng gia tốc phát hiện ngã (g)
#define FALL_DETECTION_WINDOW 500  // Cửa sổ thời gian phát hiện ngã (ms)
#define BUZZER_DURATION 5000       // Thời gian kêu còi (ms)
...
unsigned long buzzerStartTime = 0;  // Thời điểm bắt đầu kêu còi
bool buzzerActive = false;  // Trạng thái còi
...
void loop() {
    ...
    if (currentMillis - lastSensorRead >= SENSOR_READ_INTERVAL) {
        ...
        // Kiểm tra phát hiện ngã
        if (sensors.lsm6ds3) {
            if (detectFall(currentData.accelX, currentData.accelY, currentData.accelZ)) {
                Serial.println("⚠️  FALL DETECTED! Activating buzzer...");
                activateBuzzer();
            }
        }
    ...
    // Kiểm tra và tắt buzzer sau khi hết thời gian
    checkBuzzer();
...
bool detectFall(float accelX, float accelY, float accelZ) {
    ...
    // Phát hiện ngã khi:
    // 1. Tổng gia tốc vượt ngưỡng (chuyển động mạnh đột ngột)
    // 2. Hoặc gia tốc Z âm mạnh (người nằm xuống)
    bool suddenMovement = totalAccel > FALL_ACCEL_THRESHOLD;
    bool lyingDown = accelZ < -1.5;  // Gia tốc Z âm mạnh
    
    if (suddenMovement || lyingDown) {
        Serial.print("Fall detection - Total Accel: ");
